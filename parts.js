...
async function renderPartProfile(arg) {
    ...
    // Inventory Details (always shown — uses None badge when qty=0)
    ...
    html += '<div class="ms-field"><span class="ms-field-label">Condition</span><span class="ms-field-val">' + (totalQty === 0 ? condBadge('None') : condBadge(topInv?.condition)) + '</span></div>';
    html += '<div class="ms-field"><span class="ms-field-label">Source</span><span class="ms-field-val">' + (topInv?.source && totalQty > 0 ? esc(topInv.source) : '-') + '</span></div>';
    html += '<div class="ms-field"><span class="ms-field-label">Date Acquired</span><span class="ms-field-val">' + (topInv?.date_acquired && totalQty > 0 ? fmtDate(topInv.date_acquired) : '-') + '</span></div>';
    if (topInv?.price_paid && totalQty > 0) html += '<div class="ms-field"><span class="ms-field-label">Paid</span><span class="ms-field-val" style="color:var(--success)">$' + topInv.price_paid + '</span></div>';
    if (topInv?.sourced_from_vehicle && totalQty > 0) html += '<div class="ms-field"><span class="ms-field-label">Intended For</span><span class="ms-field-val">' + esc(topInv.sourced_from_vehicle) + '</span></div>';
